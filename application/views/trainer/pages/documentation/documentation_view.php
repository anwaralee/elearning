...
<form action="<?php echo base_url().'documentation'?>" method="post" id="register">
...
if($course != NULL)
{
		$c = mysql_fetch_assoc($course);
		
		$cat = $this->dashboard_model->getCategoryTitle($c['cat_id']);
...
<!--            <h3>Resource:</h3>-->
			...
			{ ?>
				<a href="<?php echo base_url().'documentation/view_video/'.$file;?>" class="btn btn-info">View Now</a>
				<?php 
			}
			else if((strcmp('pdf',strtolower( $type))==0) || (strcmp('slideshow',strtolower( $type))==0) || (strcmp('document',strtolower( $type))==0))
			{?>
                <a href="<?php echo base_url().'documentation/view_course/'.$file;?>" class="btn btn-info">View Now</a>
            <?php
			}
			else if(strcmp('youtube video',strtolower( $type))==0)
			{?>
                <a href="<?php echo base_url().'documentation/view_course_content/'.$co['course_id'];?>" class="btn btn-info">View Now</a>
            <?php
			}
			...
			if($co['download']==1)
			{
			?><a href="<?php echo base_url()."documentation/download/".$co['course_name']."/".$c_id."/".$type;?>" class="btn btn-success">Download</a>
            <?php }?>
        </div>
        <div class="seperator"></div>
        <?php
